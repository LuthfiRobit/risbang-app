<script>
    $(document).ready(function() {
        // Buat URL dengan id
        id = $("#fm_submit_pelaksanaan_penelitian").data('ta');
        const detailBaseUrl = '{{ route('proposal.pelaksanaan.deadline', [':id']) }}';
        ...
        DataManager.fetchData(detail)
            .then(function(response) {
                if (response.success) {
                                    ...
                                    <div class="timeline-item pt-3">
                                        ...
                                        <div class="timeline-desc timeline-desc-light-primary">
                                            ...
                                            <p class="font-weight-normal text-dark-50">
                                                DEADLINE LAPORAN PELAKSANAAN ${item.jenis.toUpperCase()}
                                            </p>
                                        </div>
                                    </div>
                                </div>`;
                    ...
                    Swal.fire('Oops...', 'Belum ada record deadline pelaksanaan', 'error');
                    var timelineItem = `
                                <div class="timeline-items mb-3 border-danger rounded border border-dashed">
                                    <div class="timeline-item pt-3">
                                        <div class="timeline-media bg-light-primary m-3">
