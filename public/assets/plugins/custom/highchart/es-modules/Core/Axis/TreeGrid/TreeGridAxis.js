...
function getTreeGridFromData(data, uniqueNames, numberOfSeries) {
    const categories = [], collapsedNodes = [], mapOfIdToNode = {},
    ...
    const treeParams = {
        ...
        before: function (node) {
            const data = isObject(node.data, true) ?
                    node.data :
                    {}, name = isString(data.name) ? data.name : '', parentNode = mapOfIdToNode[node.parent],
...
function onBeforeRender(e) {
    ...
    axes.filter(function (axis) {
        ...
        const options = axis.options || {}, labelOptions = options.labels, uniqueNames = options.uniqueNames,
            max = options.max,
...
function wrapGenerateTick(proceed, pos) {
    const axis = this, mapOptionsToLevel = axis.treeGrid.mapOptionsToLevel || {},
...
class TreeGridAxisAdditions {
     ...
     */
    getTickPositions() {
        const axis = this.axis, roundedMin = Math.floor(axis.min / axis.tickInterval) * axis.tickInterval,
