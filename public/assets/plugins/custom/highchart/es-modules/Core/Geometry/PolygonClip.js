...
const intersection = (clipEdge1, clipEdge2, prevPoint, currentPoint) => {
    const dc = [
        ...
        n2 = prevPoint[0] * currentPoint[1] - prevPoint[1] * currentPoint[0], n3 = 1 / (dc[0] * dp[1] - dc[1] * dp[0]),
        intersection = [
            (n1 * dp[0] - n2 * dc[0]) * n3,
            (n1 * dp[1] - n2 * dc[1]) * n3
        ];
...
(function (PolygonClip) {
    ...
    PolygonClip.clipPolygon = (subjectPolygon, boundsPolygon, closed = true) => {
        let clipEdge1 = boundsPolygon[boundsPolygon.length - 1], clipEdge2, prevPoint, currentPoint,
