...
class Pointer {
     ...
     */
    drag(e) {
        const chart = this.chart, chartOptions = chart.options.chart, plotLeft = chart.plotLeft,
            plotTop = chart.plotTop, plotWidth = chart.plotWidth, plotHeight = chart.plotHeight,
            mouseDownX = (this.mouseDownX || 0), mouseDownY = (this.mouseDownY || 0),
                ...
                chart.axes.forEach(function (axis) {
                    if (axis.zoomEnabled &&
                        ...
                        const horiz = axis.horiz, minPixelPadding = e.type === 'touchend' ?
                            ...
                            selectionMax = axis.toValue((horiz ? x + width : y + height) -
                                minPixelPadding);
    ...
    findNearestKDPoint(series, shared, e) {
        ...
        /** @private */
        function sort(p1, p2) {
            const isCloserX = p1.distX - p2.distX, isCloser = p1.dist - p2.dist,
     ...
     */
    pinch(e) {
        const self = this, chart = self.chart, pinchDown = self.pinchDown, touches = (e.touches || []),
        ...
        if (e.type === 'touchstart') {
            ...
            chart.axes.forEach(function (axis) {
                if (axis.zoomEnabled) {
                    const bounds = chart.bounds[axis.horiz ? 'h' : 'v'], minPixelPadding = axis.minPixelPadding,
    ...
    pinchTranslateDirection(horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch, forcedScale) {
        const chart = this.chart, xy = horiz ? 'x' : 'y', XY = horiz ? 'X' : 'Y', sChartXY = ('chart' + XY),
        ...
        let selectionWH, selectionXY, clipXY, scale = forcedScale || 1, touch0Now = touches[0][sChartXY],
    ...
    reset(allowMove, delay) {
        const pointer = this, chart = pointer.chart, hoverSeries = chart.hoverSeries, hoverPoint = chart.hoverPoint,
    ...
    runPointActions(e, p, force) {
        const pointer = this, chart = pointer.chart, series = chart.series,
