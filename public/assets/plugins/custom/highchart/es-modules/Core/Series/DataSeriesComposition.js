...
function wrapSeriesGeneratePoints(proceed) {
    ...
    const PointClass = this.pointClass, cropStart = this.cropStart || 0, data = this.data || [], points = [],
        processedXData = this.processedXData, processedYData = this.processedYData;
    let cursor, point;
...
class DataSeriesAdditions {
    ...
    setTable(table, redraw = true, animation) {
        const series = this.series, anySeries = series, oldData = series.points, keys = series.parallelArrays,
