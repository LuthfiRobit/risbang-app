...
class Point {
    ...
    animateBeforeDestroy() {
        const point = this, animateParams = {x: point.startXPos, opacity: 0},
    ...
    destroy() {
        if (!this.destroyed) {
            const point = this, series = point.series, chart = series.chart, dataSorting = series.options.dataSorting,
    ...
    optionsToObject(options) {
        const series = this.series, keys = series.options.keys, pointArrayMap = keys || series.pointArrayMap || ['y'],
    ...
    tooltipFormatter(pointFormat) {
        ...
        const series = this.series, seriesTooltipOptions = series.tooltipOptions,
    ...
    update(options, redraw, animation, runEvent) {
        const point = this, series = point.series, graphic = point.graphic, chart = series.chart,
     ...
     */
    setState(state, move) {
        const point = this, series = point.series, previousState = point.state,
        ...
        let halo = series.halo, markerAttribs, pointAttribs, pointAttribsAnimation,
