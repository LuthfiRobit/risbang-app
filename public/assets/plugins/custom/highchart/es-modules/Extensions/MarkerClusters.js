...
symbols.cluster = function (x, y, width, height) {
    const w = width / 2, h = height / 2, outerWidth = 1, space = 1,
...
Scatter.prototype.animateClusterPoint = function (clusterObj) {
    const series = this, chart = series.chart, mapView = chart.mapView, clusterOptions = series.options.cluster,
    ...
    let parentId, oldPointObj, newPointObj, newPointBBox, offset = 0, newX = 0, newY = 0, isOldPointGrahic = false,
        isCbHandled = false;
...
Scatter.prototype.getScaledGridSize = function (options) {
    const series = this, xAxis = series.xAxis, mapView = this.chart.mapView,
...
Scatter.prototype.getRealExtremes = function () {
    const chart = this.chart, x = chart.mapView ? 0 : chart.plotLeft, y = chart.mapView ? 0 : chart.plotTop,
...
Scatter.prototype.onDrillToCluster = function (event) {
    ...
    point.firePointEvent('drillToCluster', event, function (e) {
        const point = e.point || e.target, series = point.series, xAxis = point.series.xAxis,
            yAxis = point.series.yAxis, chart = point.series.chart, mapView = chart.mapView,
            clusterOptions = series.options.cluster, drillToCluster = (clusterOptions || {}).drillToCluster;
        if (drillToCluster && point.clusteredData) {
            const sortedDataX = point.clusteredData
                    .map((data) => data.x)
                    .sort((a, b) => a - b), sortedDataY = point.clusteredData
                    .map((data) => data.y)
                    .sort((a, b) => a - b), minX = sortedDataX[0], maxX = sortedDataX[sortedDataX.length - 1],
                minY = sortedDataY[0], maxY = sortedDataY[sortedDataY.length - 1],
                offsetX = Math.abs((maxX - minX) * 0.1), offsetY = Math.abs((maxY - minY) * 0.1),
                x1 = Math.min(minX, maxX) - offsetX, x2 = Math.max(minX, maxX) + offsetX,
                y1 = Math.min(minY, maxY) - offsetY, y2 = Math.max(minY, maxY) + offsetY;
            if (mapView) {
...
Scatter.prototype.preventClusterCollisions = function (props) {
    let series = this, [gridY, gridX] = props.key.split('-').map(parseFloat), gridSize = props.gridSize,
        groupedData = props.groupedData, defaultRadius = props.defaultRadius, clusterRadius = props.clusterRadius,
        gridXPx = gridX * gridSize, gridYPx = gridY * gridSize, propsPx = valuesToPixels(series, props),
        xPixel = propsPx.x, yPixel = propsPx.y, gridsToCheckCollision = [], pointsLen = 0, radius = 0,
...
Scatter.prototype.hideClusteredData = function () {
    const series = this, clusteredSeriesData = this.markerClusterSeriesData,
        oldState = ((series.markerClusterInfo || {}).pointsState || {}).oldState || {},
        oldPointsId = oldPointsStateId.map((elem) => (oldState[elem].point || {}).id || '');
    (clusteredSeriesData || []).forEach(function (point) {
...
Scatter.prototype.generatePoints = function () {
    const series = this, chart = series.chart, mapView = chart.mapView, xData = series.xData, yData = series.yData,
        clusterOptions = series.options.cluster, realExtremes = series.getRealExtremes(), visibleXData = [],
        visibleYData = [], visibleDataIndexes = [];
    let oldPointsState, oldDataLen, oldMarkerClusterInfo, kmeansThreshold, cropDataOffsetX, cropDataOffsetY, seriesMinX,
        seriesMaxX, seriesMinY, seriesMaxY, type, algorithm, clusteredData, groupedData, layoutAlgOptions, point, i;
    ...
    if (clusterOptions &&
        ...
        const halfGrid = layoutAlgOptions.processedGridSize / 2, p1 = pixelsToValues(series, {x: 0, y: 0}),
...
addEvent(Chart, 'render', function () {
    ...
    (chart.series || []).forEach(function (series) {
        if (series.markerClusterInfo) {
            const options = series.options.cluster, pointsState = (series.markerClusterInfo || {}).pointsState,
...
addEvent(Point, 'drillToCluster', function (event) {
    const point = event.point || event.target, series = point.series, clusterOptions = series.options.cluster,
