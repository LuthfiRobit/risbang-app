...
class SonificationTimeline {
    ...
    play(filter, filterPersists = true, resetAfter = true, onEnd) {
        ...
        this.isPaused = false;
        this.isPlaying = true;
        const skipThreshold = this.options.skipThreshold || 2, onPlay = this.options.onPlay,
            showTooltip = this.options.showTooltip, showCrosshair = this.options.showCrosshair, channels = filter ?
        ...
        channels.forEach((channel) => {
            ...
            for (let i = 0; i < numEvents; ++i) {
                ...
                const point = e.relatedPoint, chart = point && point.series && point.series.chart,
    ...
    playSegment(segment, onEnd) {
        ...
        if (eventTimes.first < Infinity) {
            const segmentSize = (eventTimes.last - eventTimes.first) / numSegments,
                fromTime = eventTimes.first + segment * segmentSize, toTime = fromTime + segmentSize;
    ...
    downloadMIDI(filename) {
        const data = this.getMIDIData(), name = (filename ||
                ...
                'chart') + '.mid', blob = new Blob([data], {type: 'application/octet-stream'}),
