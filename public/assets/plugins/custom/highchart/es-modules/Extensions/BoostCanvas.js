...
const initCanvasBoost = function () {
    if (H.seriesTypes.heatmap) {
        wrap(H.seriesTypes.heatmap.prototype, 'drawPoints', function () {
            const chart = this.chart, ctx = this.getContext(), inverted = this.chart.inverted, xAxis = this.xAxis,
                yAxis = this.yAxis;
            if (ctx) {
                // draw the columns
    ...
    extend(Series.prototype, {
         ...
         */
        getContext: function () {
            const chart = this.chart, target = isChartSeriesBoosting(chart) ? chart : this,
                targetGroup = (target === chart ?
                    chart.seriesGroup :
                    chart.seriesGroup || this.group);
            let width = chart.chartWidth, height = chart.chartHeight, ctx,
        ...
        renderCanvas: function () {
            let series = this, options = series.options, chart = series.chart, xAxis = this.xAxis, yAxis = this.yAxis,
                ...
                enableMouseTracking = options.enableMouseTracking, lastPoint, threshold = options.threshold,
            ...
            BoostSeries.eachAsync(sdata, function (d, i) {
                let x, y, clientX, plotY, isNull, low, isNextInside = false, isPrevInside = false, nx = false,
