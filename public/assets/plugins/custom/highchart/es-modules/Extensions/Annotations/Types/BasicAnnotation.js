...
class BasicAnnotation extends Annotation {
     ...
     * */
    addControlPoints() {
        const options = this.options, controlPoints = BasicAnnotation.basicControlPoints,
            annotationType = this.basicType, optionsGroup = (options.labels ||
                options.shapes ||
                []);
...
BasicAnnotation.basicControlPoints = {
    ...
    rectangle: [{
        ...
        events: {
            drag: function (e, target) {
                var _a, _b;
                const annotation = target.annotation, coords = this.chart.pointer.getCoordinates(e),
                    points = target.options.points, shapes = annotation.userOptions.shapes,
                    ...
                    x = coords.xAxis[xAxisIndex].value, y = coords.yAxis[yAxisIndex].value;
                // Top right point
                points[1].x = x;
    ...
    circle: [{
        ...
        events: {
            ...
            // TRANSLATION
            drag: function (e, target) {
                const annotation = target.annotation, position = this.mouseMoveToTranslation(e),
    ...
    ellipse: [{
        ...
        positioner: function (target) {
            const position = target.getAbsolutePosition(target.points[0]),
                position2 = target.getAbsolutePosition(target.points[1]), attrs = target.getAttrs(position, position2);
            return {
