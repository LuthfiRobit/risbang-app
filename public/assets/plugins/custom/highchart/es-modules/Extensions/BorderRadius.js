...
if (SVGElement.symbolCustomAttribs.indexOf('borderRadius') === -1) {
    ...
    SVGRenderer.prototype.symbols.arc = function (x, y, w, h, options = {}) {
        ...
        const alpha = end - start, sinHalfAlpha = Math.sin(alpha / 2),
    ...
    SVGRenderer.prototype.symbols.roundedRect = function (x, y, width, height, options = {}) {
        const path = roundedRect(x, y, width, height, options), {r = 0, brBoxHeight = height, brBoxY = y} = options,
            brOffsetTop = y - brBoxY, brOffsetBtm = (brBoxY + brBoxHeight) - (y + height),
            // When the distance to the border-radius box is greater than the r
            // itself, it means no border radius. The -0.1 accounts for float
            // rounding errors.
            rTop = (brOffsetTop - r) > -0.1 ? 0 : r, rBtm = (brOffsetBtm - r) > -0.1 ? 0 : r,
            cutTop = Math.max(rTop && brOffsetTop, 0), cutBtm = Math.max(rBtm && brOffsetBtm, 0);
        /*
    ...
    addEvent(Series, 'afterColumnTranslate', function () {
        ...
        if (this.options.borderRadius &&
            ...
            const {options, yAxis} = this, percent = options.stacking === 'percent',
