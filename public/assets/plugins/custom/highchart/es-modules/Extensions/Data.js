...
class Data {
     ...
     */
    parseCSV(inOptions) {
        const self = this, columns = this.columns = [], options = inOptions || this.options,
     ...
     */
    parseTable() {
        const options = this.options, columns = this.columns || [], startRow = options.startRow || 0,
     ...
     */
    fetchLiveData() {
        const data = this, chart = this.chart, options = this.options, maxRetries = 3,
            pollingEnabled = options.enablePolling, originalOptions = merge(options);
    ...
    parseGoogleSpreadsheet() {
        const data = this, options = this.options, googleSpreadsheetKey = options.googleSpreadsheetKey,
    ...
    parseColumn(column, col) {
        const rawColumns = this.rawColumns, columns = this.columns, firstRowAsNames = this.firstRowAsNames,
            isXColumn = this.valueCount.xColumns.indexOf(col) !== -1, backup = [], chartOptions = this.chartOptions,
            columnTypes = this.options.columnTypes || [], columnType = columnTypes[col],
