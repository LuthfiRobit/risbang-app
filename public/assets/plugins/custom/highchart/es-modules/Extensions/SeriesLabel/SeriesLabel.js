...
function checkClearPoint(series, x, y, bBox, checkDistance) {
    const chart = series.chart, seriesLabelOptions = series.options.label || {},
        onArea = pick(seriesLabelOptions.onArea, !!series.area),
        findDistanceToOthers = (onArea || seriesLabelOptions.connectorAllowed), leastDistance = 16,
...
function drawSeriesLabels(chart) {
    ...
    chart.series.forEach((s) => (s.points || []).forEach((p) => (p.dataLabels || []).forEach((label) => {
        const {width, height} = label.getBBox(), left = label.translateX + (s.xAxis ? s.xAxis.pos : s.chart.plotLeft),
    ...
    chart.series.forEach(function (series) {
        ...
        const colorClass = ('highcharts-color-' + pick(series.colorIndex, 'none')), isNew = !series.labelBySeries,
            ...
            onArea = pick(labelOptions.onArea, !!series.area), results = [], xData = series.xData || [];
        ...
        function insidePane(x, y, bBox) {
            const leftBound = Math.max(paneLeft, pick(areaMin, -Infinity)),
...
function getPointsOnGraph(series) {
    ...
    const distance = 16, points = series.points, interpolated = [], graph = series.graph || series.area,
        node = graph && graph.element, inverted = series.chart.inverted, xAxis = series.xAxis, yAxis = series.yAxis,
        paneLeft = inverted ? yAxis.pos : xAxis.pos, paneTop = inverted ? xAxis.pos : yAxis.pos,
        seriesLabelOptions = series.options.label || {}, onArea = pick(seriesLabelOptions.onArea, !!series.area),
        translatedThreshold = yAxis.getThreshold(series.options.threshold), grid = {};
...
function onChartRedraw(e) {
    if (this.renderer) {
        ...
        chart.series.forEach(function (series) {
            const seriesLabelOptions = series.options.label || {}, label = series.labelBySeries,
