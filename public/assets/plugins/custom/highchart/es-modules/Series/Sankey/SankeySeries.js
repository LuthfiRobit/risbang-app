...
class SankeySeries extends ColumnSeries {
    ...
    pointAttribs(point, state) {
        ...
        const series = this, level = point.isNode ? point.level : point.fromNode.level,
    ...
    translate() {
        ...
        const series = this, chart = this.chart, options = this.options, nodeWidth = this.nodeWidth,
    ...
    translateLink(point) {
        ...
        let fromNode = point.fromNode, toNode = point.toNode, chart = this.chart,
        ...
        if (straight && typeof toY === 'number') {
            ...
            const bend = 20, vDist = chart.plotHeight - fromY - linkHeight, x1 = right - bend - linkHeight,
    ...
    translateNode(node, column) {
        const translationFactor = this.translationFactor, chart = this.chart, options = this.options, {
                borderRadius,
                borderWidth = 0
            } = options, sum = node.getSum(),
            nodeHeight = Math.max(Math.round(sum * translationFactor), this.options.minLinkWidth),
            nodeWidth = Math.round(this.nodeWidth), crisp = Math.round(borderWidth) % 2 / 2,
        ...
        if (sum) {
            ...
            let x = nodeLeft, y = fromNodeTop, width = node.options.width || options.width || nodeWidth,
