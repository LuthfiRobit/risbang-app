...
function onHeikinAshiSeriesAfterTranslate() {
    const series = this, points = series.points, heikiashiData = series.heikiashiData,
...
class HeikinAshiSeries extends CandlestickSeries {
     ...
     */
    getHeikinashiData() {
        const series = this, processedYData = series.allGroupedData || series.yData,
    ...
    modifyDataPoint(dataPoint, previousDataPoint) {
        const newOpen = (previousDataPoint[0] + previousDataPoint[3]) / 2, newClose = (dataPoint[0] +
                dataPoint[1] +
                dataPoint[2] +
                dataPoint[3]) / 4, newHigh = Math.max(dataPoint[1], newClose, newOpen),
