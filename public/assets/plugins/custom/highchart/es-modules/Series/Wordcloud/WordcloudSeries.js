...
class WordcloudSeries extends ColumnSeries {
    ...
    deriveFontSize(relativeWeight, maxFontSize, minFontSize) {
        const weight = isNumber(relativeWeight) ? relativeWeight : 0, max = isNumber(maxFontSize) ? maxFontSize : 1,
            min = isNumber(minFontSize) ? minFontSize : 1;
        return Math.floor(Math.max(min, weight * max));
    }

    drawPoints() {
        let series = this, hasRendered = series.hasRendered, xAxis = series.xAxis, yAxis = series.yAxis,
