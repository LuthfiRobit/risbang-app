...
function isPolygonsOverlappingOnAxis(axis, polygon1, polygon2) {
    const projection1 = project(polygon1, axis), projection2 = project(polygon2, axis),
...
function isPolygonsColliding(polygon1, polygon2) {
    ...
    const axes1 = getAxesFromPolygon(polygon1), axes2 = getAxesFromPolygon(polygon2), axes = axes1.concat(axes2),
        overlappingOnAllAxes = !find(axes, (axis) => isPolygonsOverlappingOnAxis(axis, polygon1, polygon2));
...
function intersectsAnyWord(point, points) {
    let intersects = false, rect = point.rect, polygon = point.polygon, lastCollidedWith = point.lastCollidedWith,
...
function archimedeanSpiral(attempt, params) {
    let field = params.field, result = false, maxDelta = (field.width * field.width) + (field.height * field.height),
...
function squareSpiral(attempt, params) {
    let a = attempt * 4, k = Math.ceil((Math.sqrt(a) - 1) / 2), t = 2 * k + 1, m = Math.pow(t, 2),
...
function intersectionTesting(point, options) {
    let placed = options.placed, field = options.field, rectangle = options.rectangle, polygon = options.polygon,
...
function getPolygon(x, y, width, height, rotation) {
    const origin = [x, y], left = x - (width / 2), right = x + (width / 2), top = y - (height / 2),
