...
class DependencyWheelSeries extends SankeySeries {
    ...
    translate() {
        const options = this.options, factor = 2 * Math.PI /
            ...
            startAngle = (options.startAngle - 90) * deg2rad, brOption = options.borderRadius,
        ...
        this.nodeColumns[0].forEach(function (node) {
            ...
            if (node.sum) {
                const shapeArgs = node.shapeArgs, centerX = center[0], centerY = center[1], r = center[2] / 2,
                    innerR = r - options.nodeWidth, start = startAngle + factor * (shapeArgs.y || 0), end = startAngle +
                ...
                node.linksFrom.forEach(function (point) {
                    if (point.linkBase) {
                        ...
                        const corners = point.linkBase.map(function (top, i) {
                            let angle = factor * top, x = Math.cos(startAngle + angle) * (innerR + 1),
                                y = Math.sin(startAngle + angle) * (innerR + 1), curveFactor = options.curveFactor || 0;
