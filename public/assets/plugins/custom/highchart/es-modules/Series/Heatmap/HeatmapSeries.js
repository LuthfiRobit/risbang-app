...
class HeatmapSeries extends ScatterSeries {
     ...
     */
    drawPoints() {
        const series = this, seriesOptions = series.options, interpolation = seriesOptions.interpolation,
            seriesMarkerOptions = seriesOptions.marker || {};
        if (interpolation) {
            const {image, chart, xAxis, yAxis, points} = series, lastPointIndex = points.length - 1, {
                    len: xAxisLen,
                    reversed: xRev
                } = xAxis, {len: yAxisLen, reversed: yRev} = yAxis, {
                    min: xMin,
                    max: xMax
                } = xAxis.getExtremes(), {min: yMin, max: yMax} = yAxis.getExtremes(), [colsize, rowsize] = [
                    pick(seriesOptions.colsize, 1),
                    pick(seriesOptions.rowsize, 1)
                ], inverted = chart.inverted, xTranslationPad = colsize / 2, userMinPadding = xAxis.userOptions.minPadding,
                isUserMinPadZero = (defined(userMinPadding) &&
                    !(userMinPadding > 0)), noOffset = (inverted || isUserMinPadZero),
                padIfMinSet = (isUserMinPadZero && xTranslationPad || 0), [x1, x2, postTranslationOffset] = [
                    xMin - padIfMinSet,
                    xMax + (padIfMinSet * 2),
                    isUserMinPadZero && 0 || (xMin + colsize)
                ].map((side) => (clamp(Math.round(xAxis.len -
                    xAxis.translate(side, false, true, false, true, -series.pointPlacementToXValue())), -xAxis.len, 2 * xAxis.len))), [xStart, xEnd] = xRev ? [x2, x1] : [x1, x2],
                xOffset = (noOffset && 0 ||
            ...
            if (!image || series.isDirtyData) {
                ...
                if (canvas && ctx && colorAxis) {
                    const canvasWidth = canvas.width = ~~((xMax - xMin) / colsize) + 1,
                        canvasHeight = canvas.height = ~~((yMax - yMin) / rowsize) + 1,
                        canvasArea = canvasWidth * canvasHeight, pixelData = new Uint8ClampedArray(canvasArea * 4),
                        widthLastIndex = (canvasWidth - (noOffset && 1 || 0)), heightLastIndex = canvasHeight - 1,
                        colorFromPoint = (p) => {
                    ...
                    for (let i = 0; i < canvasArea; i++) {
                        const toPointScale = scaleToImg(i * 4, 0, pixelData.length - 4, 0, lastPointIndex),
                            p = points[toPointScale], sourceArr = new Uint8ClampedArray(colorFromPoint(p));
    ...
    markerAttribs(point, state) {
        ...
        if (state && state !== 'normal') {
            const pointMarkerOptions = point.options.marker || {}, seriesMarkerOptions = this.options.marker || {},
    ...
    pointAttribs(point, state) {
        const series = this, attr = Series.prototype.pointAttribs.call(series, point, state),
            seriesOptions = series.options || {}, plotOptions = series.chart.options.plotOptions || {},
            seriesPlotOptions = plotOptions.series || {}, heatmapPlotOptions = plotOptions.heatmap || {},
     ...
     */
    translate() {
        const series = this, options = series.options, {borderRadius, marker} = options,
            symbol = marker && marker.symbol || 'rect', shape = symbols[symbol] ? symbol : 'rect',
            hasRegularShape = ['circle', 'square'].indexOf(shape) !== -1;
        series.generatePoints();
        series.points.forEach(function (point) {
            const cellAttr = point.getCellAttributes();
            let x = Math.min(cellAttr.x1, cellAttr.x2), y = Math.min(cellAttr.y1, cellAttr.y2),
