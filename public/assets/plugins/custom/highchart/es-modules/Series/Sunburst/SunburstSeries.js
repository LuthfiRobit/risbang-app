...
function getDlOptions(params) {
    ...
    let point = params.point, shape = isObject(params.shapeArgs) ? params.shapeArgs : {},
...
function getAnimation(shape, params) {
    let point = params.point, radians = params.radians, innerR = params.innerR, idRoot = params.idRoot,
...
function cbSetTreeValuesBefore(node, options) {
    const mapIdToNode = options.mapIdToNode, parent = node.parent, nodeParent = parent ? mapIdToNode[parent] : void 0,
        series = options.series, chart = series.chart, points = series.points, point = points[node.i],
...
class SunburstSeries extends TreemapSeries {
    ...
    drawPoints() {
        let series = this, mapOptionsToLevel = series.mapOptionsToLevel, shapeRoot = series.shapeRoot,
        ...
        points.forEach(function (point) {
            let node = point.node, level = mapOptionsToLevel[node.level], shapeExisting = (point.shapeExisting || {}),
    ...
    layoutAlgorithm(parent, children, options) {
        let startAngle = parent.start, range = parent.end - startAngle, total = parent.val, x = parent.x, y = parent.y,
            radius = ((options &&
        ...
        return (children || []).reduce(function (arr, child) {
            const percentage = (1 / total) * child.val, radians = percentage * range,
    ...
    setShapeArgs(parent, parentValues, mapOptionsToLevel) {
        ...
        children.forEach(function (child, index) {
            const values = childrenValues[index], angle = values.start + ((values.end - values.start) / 2),
    ...
    translate() {
        let series = this, options = series.options, positions = series.center = series.getCenter(),
