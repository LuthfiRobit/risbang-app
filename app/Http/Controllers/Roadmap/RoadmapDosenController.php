...
class RoadmapDosenController extends Controller
{
    ...
    public function index()
    {
        ...
        $roadmapProdi = [];
        if ($this->dosenRole == 'dosen') {
            $roadmapProdi = RoadMap::where('prodi_id', Auth::user()->dosen->prodi_id)->get()
                ->map(function ($q) {
                    return [
                        'nama_roadmap' => $q->nama_roadmap,
                        'berkas' => asset('files/berkasRoadmap') . '/' . $q->berkas
                    ];
                });
        }
        // return $roadmapProdi;
        return view('roadmap.dosen.index', compact('rentanWaktu', 'roadmapProdi'));
    }
    ...
    public function list()
    {
        $data = RoadMap::select(
            ...
            ->leftJoin('dosen', 'dosen.id_dosen', '=', 'roadmap.dosen_id')
            ...
            //auth dosen
            ->when($this->dosenRole == 'dosen', function ($q) {
                $q->where('roadmap.dosen_id', $this->dosenId);
            })
            // auth kaprodi
            ->when($this->dosenRole == 'kaprodi', function ($q) {
                $q->where('dosen.prodi_id', $this->kaprodiId);
            })
            // auth kaprodi
    ...
    public function store(Request $request)
    {
        ...
        $file_name = 'FRD_' . uniqid() . '.' . $file->getClientOriginalExtension();
        ...
        try {
            ...
            DB::rollBack();
            Log::error('Create roadmap dosen: ' . $th->getMessage());
