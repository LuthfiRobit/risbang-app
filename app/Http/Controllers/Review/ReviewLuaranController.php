...
use App\Models\ReviewLuaranProposal;
...
class ReviewLuaranController extends Controller
{
    ...
    {
        return view('review.luaran.index');
    }
    ...
    public function viewDosen(string $id)
    {
        ...
        $tahunAkademik = TahunAkademik::select('nama_tahun_akademik')
            ...
            ->first();
        return view('review.luaran.indexDsn', compact('tahunAkademik'));
    }
    ...
    public function listDsn(string $id)
    {
        ...
        $data = DB::table("detail_reviewer")
            ...
            ->leftJoin("luaran_proposal as k_penelitian", function ($join) {
            ...
            ->leftJoin("luaran_proposal as k_pengabdian", function ($join) {
    ...
    public function overview(Request $request, string $id)
    {
        ...
        // return $data;
        return view('review.luaran.overview', $data);
    }

    public function store(Request $request)
    {
        $id = Crypt::decryptString($request->luaran_id);
        ...
        DB::beginTransaction();
        try {
            $save['luaran_proposal_id'] = $id;
            $save['reviewer_id'] = $this->reviewerId;
            $save['komen'] = $request->komen;
            $save['nilai'] = $request->nilai;

            $store = ReviewLuaranProposal::create($save);
            if ($store) {
                LuaranProposal::where('id_luaran_proposal', $id)->update(['status_review' => $request->status_review]);
            }
            ...
            DB::rollBack();
            Log::error('Create review luaran proposal: ' . $th->getMessage());
    ...
    {
        $id = Crypt::decryptString($id);
        $one = DB::table('review_luaran_proposal')
            ->select(
                'luaran_proposal_id',
                'komen',
                'nilai',
            ...
            ->leftJoin('luaran_proposal', 'luaran_proposal.id_luaran_proposal', '=', 'review_luaran_proposal.luaran_proposal_id')
            ->where('luaran_proposal_id', $id)->latest('review_luaran_proposal.created_at')->first();
        if ($one) {
